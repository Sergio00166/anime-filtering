...
def build_rescale(src, kernel, sharp=False):
    ...
    rb = rb.errormask(mask=0.0275, expand=2)
    rb = rb.linemask()
    if sharp:
        # Keep at least same sharpening as source
        # And normalize as debilinear is too sharp
        rb = rb.post_double(lambda x:\
            core.cas.CAS(x, sharpness=0.25))
...
builders = [
    ...
    build_rescale(src, Lanczos, sharp=True),
]
